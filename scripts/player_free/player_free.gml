function player_free(){
	...
	if hspd != 0 {
		...
		if on_ground {
			if hspd > 2.9 {
			repeat(2) {instance_create_depth(x-(4*image_xscale),y+12,12,part_runR);}
			}
			if hspd < -2.9 {
			repeat(2) {instance_create_depth(x-(4*image_xscale),y+12,12,part_runL);}
			}
		}
	} else {
		image_speed = 0;	
	   ...
	   if global.age >= 2 and global.age <= 5 {
			...
			//Jumping
			 if (global.J) {				// if we press the jump key
				...
				buff = 0;
				audio_sound_gain(sfx_jump,global.SFXVOL,0);
	    ...
	    #region Double Jumping AND testing for jump-buffer
			...
			if (global.J && candj) and buff < 0 {	
				...
				audio_sound_gain(sfx_djump,global.SFXVOL,0);
