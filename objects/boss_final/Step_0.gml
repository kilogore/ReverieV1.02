...
switch (state) {
	...
	if toss = true {
		toss = false;
			...
			aa.speed = 5;
			aa.gravity = 0.1;
				audio_sound_gain(sfx_laser1,0.4*global.SFXVOL,0);
	...
	if laserOn = true {
		boss_laser();
		if !audio_is_playing(sfx_laser3) {
				audio_sound_gain(sfx_laser3,global.SFXVOL,0);
				audio_play_sound(sfx_laser3,0,false);}
	}
	...
	flex = true;
		...
		repeat(3) {
			...
			audio_sound_gain(sfx_laser2,0.4*global.SFXVOL,0);
	...
	if HP2 <= 0 {
			state = final.dead;
				audio_sound_gain(sfx_boss_boom,global.SFXVOL,0);
