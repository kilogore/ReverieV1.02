...
switch (state) {
	...
	if instance_exists(boss_joint2) {
		...
		scribble("[fnt_title][fa_center][c_white]"+string(msg)).draw(x,y-48);

	break;
	
	case final.phase2:
		scribble("[fnt_title][fa_center][c_white]"+string(msg2)).draw(x,y-48);
	break;
	
	case final.phase3:
		if fastRoll = true {
			scribble("[fnt_title][fa_center][c_white]"+string(msg3)).draw(x,y-48);
		} else {
			scribble("[fnt_title][fa_center][c_white]"+string(msg4)).draw(x,y-48);
		}
	break;
